...
object PdfGenerator {
    ...
    fun generatePdf(
        ...
        for (cat in stats.topCategories) {
            ...
            val pctStr = String.format("%s%.1f%%", pctPrefix, cat.percentageChange.toDouble())
            ...
            categoriesHtml.append("""
                <div class="category-item">
                    ...
                    <div class="category-right">
                        ...
                        <span class="category-change $changeClass">$pctStr</span>
                    </div>
                    <div class="clear"></div>
                </div>
            """.trimIndent())
    ...
    fun generateDetailedPdf(
        ...
        val htmlContent = java.lang.StringBuilder()
        htmlContent.append("""
            <!DOCTYPE html>
            <html dir="$dirAttr">
            <head>
        ...
        while (hasMore) {
            val pageRequest = org.springframework.data.domain.PageRequest.of(page, size, org.springframework.data.domain.Sort.by("transactionDate").descending())
            ...
            for (t in transactions.content) {
                if (dataType == DataType.EXPENSES && t.amount >= java.math.BigDecimal.ZERO) continue
                if (dataType == DataType.INCOME && t.amount < java.math.BigDecimal.ZERO) continue

                val typeStr = t.type.name.lowercase()
                val amountClass = if (t.amount >= java.math.BigDecimal.ZERO) "amount-in" else "amount-out"
